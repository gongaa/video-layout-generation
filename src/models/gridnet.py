...
class GridNet(nn.Module):
    ...
    def __init__(self, n_channels, seg_out = 20, img_out = 3, filters_level = [32, 64, 96],
        ...
        self.lateral_out_seg = LateralBlock(in_ch=self.f_level[0], out_ch=seg_out,
                                       shortcut_conv=True, name='lateral_out_seg')
        self.lateral_out_img = LateralBlock(in_ch=self.f_level[0], out_ch=img_out,
                                       shortcut_conv=True, name='lateral_out_img') 
...
class CoordGridNet(nn.Module):
    ...
    def __init__(self, n_channels, seg_out = 20, img_out = 3, filters_level = [32, 64, 96],
        ...
        self.lateral_in = CoordLateralBlock(in_ch=n_channels, out_ch=self.f_level[0],
                                       shortcut_conv=True, name='lateral_in')
        self.lateral_out_seg = CoordLateralBlock(in_ch=self.f_level[0], out_ch=seg_out,
                                       shortcut_conv=True, name='lateral_out_seg')
        self.lateral_out_img = CoordLateralBlock(in_ch=self.f_level[0], out_ch=img_out,
                                       shortcut_conv=True, name='lateral_out_img') 
        setattr(self, 'down_00', CoordDownSamplingBlock(in_ch=self.f_level[0], out_ch=self.f_level[1], name='down_00'))
        setattr(self, 'down_10', CoordDownSamplingBlock(in_ch=self.f_level[1], out_ch=self.f_level[2], name='down_10'))

        for i in range(1, int(self.n_col/2)):
            setattr(self, f'lateral_0{i-1}',CoordLateralBlock(in_ch=self.f_level[0], out_ch=self.f_level[0], name=f'lateral_0{i-1}'))
            setattr(self, f'down_0{i}', CoordDownSamplingBlock(in_ch=self.f_level[0], out_ch=self.f_level[1], name=f'down_0{i}'))
            setattr(self, f'down_1{i}', CoordDownSamplingBlock(in_ch=self.f_level[1], out_ch=self.f_level[2], name=f'down_1{i}'))
            setattr(self, f'lateral_1{i-1}', CoordLateralBlock(in_ch=self.f_level[1], out_ch=self.f_level[1], name=f'lateral_1{i-1}'))
            setattr(self, f'lateral_2{i-1}', CoordLateralBlock(in_ch=self.f_level[2], out_ch=self.f_level[2], name=f'lateral_2{i-1}'))

        for i in range(int(self.n_col/2), self.n_col):
            setattr(self, f'lateral_2{i-1}', CoordLateralBlock(in_ch=self.f_level[2], out_ch=self.f_level[2], name = f'lateral_2{i-1}'))
            setattr(self, f'lateral_1{i-1}', CoordLateralBlock(in_ch=self.f_level[1], out_ch=self.f_level[1], name = f'lateral_1{i-1}'))
            setattr(self, f'lateral_0{i-1}', CoordLateralBlock(in_ch=self.f_level[0], out_ch=self.f_level[0], name = f'lateral_0{i-1}'))
            setattr(self, f'up_1{i}', CoordUpSamplingBlock(in_ch=self.f_level[2], out_ch=self.f_level[1], name=f'up_1{i}')) # when use, use self.up_1[i-int(self.n_col/2)]
            setattr(self, f'up_0{i}', CoordUpSamplingBlock(in_ch=self.f_level[1], out_ch=self.f_level[0], name=f'up_0{i}')) # when use, use self.up_0[i-int(self.n_col/2)]
